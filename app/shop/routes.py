...
from app import db
from app.models import Shopkeeper, Product, Sale, StockMovement, Expense, OnlineTransaction
import razorpay
...
def add_product():
    ...
    if request.method == "POST":
        ...
        if image_file and image_file.filename:
            ...
            unique_name = f"{shop.id}_{int(time.time())}_{filename}"
            file_path = os.path.join(upload_folder, unique_name)
            image_file.save(file_path)

            p.image_filename = unique_name
